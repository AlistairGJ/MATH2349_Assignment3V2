...
This attribute contained approximately 40% NA values and was dropped.
...
```{r}
gvd_acs_join_gather <- gvd_acs_join %>% gather(age_0_15:age_81_100,
                                               key = "AgeGroup", value = "AgeGroupCount")
```
...
```{r}
ggplot(gvd_acs_age_long, aes(x=AgeGroup)) + geom_bar(aes(fill=County), position = "dodge")
```
...
```{r}
ggplot(gvd_acs_age_long, aes(x=AgeGroup)) + geom_scatt (aes(fill=County))
```



```{r}
ggplot(data = censusVizNew[censusVizNew$Engagement=="Fully engaged",], aes(x = as.numeric(Age), y = Number, colour = Engagement)
```
...
density(gvd_acs_join_gather$Events)
