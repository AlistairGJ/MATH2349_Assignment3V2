...
The libraries `readr`, `diplyr`, `stringr`, `knitr`, `tidyr`, `lubridate`, `kableExtra` and `outliers` were imported into the RStudio interactive development environment. The .csv files “gun-violence-data” (Gun Violence Data, kaggle.com) and “American Census Data 2015” (US Census Demographic and Economic Data, kaggle.com) were imported as `gvd` and `acs2015`, respectively. These data were then checked for structure, including dimensions and attribute classes. It was noted the `gvd` dataset failed to conform to the “tidy data” principals. Most notably, every instance of the attributes prefixed “participant_” contained multiple values, delimited by a combination of [:digit:], :: and/or ||. In order to be computationally efficient, it was decided to first subset `gvd` before cleaning the aforementioned “participant_” attributes. 
...
aggregate(city_or_county ~ state, gvd2015, function(x) length(unique(x))) %>% 
  kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"),  font_size = 11) %>% scroll_box(height = "200px")
```
